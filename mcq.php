...
class PDF_MCQ_Plugin
{
    ...
    public function mcq_settings_meta_box_callback($post)
    {
        ...
        $number_of_answers = get_post_meta($post->ID, '_number_of_answers', true) ?: 4; // Default 4 (A-D)
        $correct_answers = get_post_meta($post->ID, '_correct_answers', true) ?: array();
        $exam_time = get_post_meta($post->ID, '_exam_time', true) ?: 30; // Default 30 minutes
    ...
    public function shortcode_handler($atts)
    {
        ...
        $exam_time = get_post_meta($quiz_id, '_exam_time', true) ?: 30; // in minutes
